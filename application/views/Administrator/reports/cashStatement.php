		...
		<div class="row" style="padding-top:10px;">
			<div class="col-md-6">
				...
				<table class="table table-bordered table-condensed">
					<thead>
						<tr style="background: #dee4dc;">
							<th colspan="5">Employee Received</th>
						</tr>
						<tr>
							<th>Employee Id</th>
							<th>Employee Name</th>
					...
					<tbody style="display:none;" v-bind:style="{display: employeeReceived.length > 0 ? '' : 'none'}">
						<tr v-for="trans in employeeReceived">
							<td>{{ trans.Employee_ID }}</td>
							<td>{{ trans.Employee_Name }}</td>
							<td>{{ trans.date }}</td>
							<td style="text-align:right;">{{ trans.received_amount | decimal }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="3" style="text-align:right">Total</td>
							<td style="text-align:right;">{{ employeeReceived.reduce((prev,curr)=> {return +prev + +curr.received_amount},0) | decimal }}</td>
						</tr>
					</tfoot>
				</table>
			</div>
			<div class="col-md-6">
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody style="display:none;" v-bind:style="{display: employeePayments.length > 0 ? '' : 'none'}">
						...
						<tr v-for="trans in employeePayments">
							<td>{{ trans.Employee_ID }}</td>
							<td>{{ trans.Employee_Name }}</td>
							<td>{{ trans.date }}</td>
							<!-- <td>{{ trans.month_name }}</td> -->
							<td style="text-align:right;">{{ trans.payment_amount | decimal }}</td>
						</tr>
						<!-- </template> -->
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
							<td colspan="3" style="text-align:right">Total</td>
							<td style="text-align:right;">{{ employeePayments.reduce((prev,curr)=> {return +prev + +curr.payment_amount},0) | decimal }}</td>
						</tr>
					</tfoot>
				</table>
...
<script>
	new Vue({
		...
		computed: {
			...
			totalEmployeeReceiveds() {
				return this.employeeReceived.reduce((prev, curr) => {
					return prev + parseFloat(curr.received_amount)
				}, 0).toFixed(2);
			},
			totalAssetsCost() {
			...
			totalCashIn() {
				return parseFloat(this.totalSales) +
					...
					parseFloat(this.totalEmployeeReceiveds);
			},
			totalCashOut() {
		...
		methods: {
			...
			getCashReceived() {
				...
				axios.post('/get_cash_transactions', filter)
					.then(res => {
						this.cashReceived = res.data.filter((obj) => {
							return obj.Acc_SlID != 9
						});
					})
			},
			...
			getEmployeePayments() {
				...
				axios.post('/get_employee_transaction', filter)
					.then(res => {
						this.employeePayments = res.data;
					})
			},
			getEmployeeReceiveds() {
				let filter = {
					dateFrom: this.filter.dateFrom,
					dateTo: this.filter.dateTo,
				...
				axios.post('/get_employee_transaction', filter)
					.then(res => {
						this.employeeReceived = res.data;
					})
			},
