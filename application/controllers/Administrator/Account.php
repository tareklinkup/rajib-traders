...
class Account extends CI_Controller
{
    ...
    public function getCashTransactions()
    {
        ...
        if (isset($data->sub_acc_id) && $data->sub_acc_id != '') {
            $accountClause .= " and sb.sub_acc_id = '$data->sub_acc_id'";
        }
        ...
        $transactions = $this->db->query("
            ...
            left join tbl_account_sub sb on sb.sub_acc_id = ct.sub_acc_id
            where ct.status = 'a'
            and ct.Tr_branchid = ?
    ...
    public function getCashLedger()
    {
        ...
        $ledger = $this->db->query("
            ...
            select 
                et.emp_trans_id as id,
                et.date as date,
                concat(et.transaction_for, ' - ',et.transaction_type) as description,
                0.00 as in_amount,
                et.payment_amount as out_amount
            from tbl_employee_transaction et
            where et.Branch_id = '$this->brunch'
            and et.status = 'a'
            and et.transaction_type = 'Payment'
            and et.date between '$data->fromDate' and '$data->toDate'
            ...
            select 
                ...
                et.date as date,
                concat(et.transaction_for, ' - ',et.transaction_type) as description,
                et.received_amount as in_amount,
                0.00 as out_amount
            from tbl_employee_transaction et
            where et.Branch_id = '$this->brunch'
            and et.status = 'a'
    ...
    */
    ...
    public function saveSubAccount()
    {
        ...
        $subAccountId = $inputObJ->sub_acc_id;

        if ($subAccountId == '') {
            $query = $this->db->query("select * from tbl_account_sub where sub_account_name = ? and branch_id = ?", [$inputObJ->sub_account_name, $this->brunch])->row();
        } else {
            $query = $this->db->query("select * from tbl_account_sub where sub_acc_id != ? and sub_account_name = ? and branch_id = ?", [$subAccountId, $inputObJ->sub_account_name, $this->brunch])->row();
        }

        if ($query) {
            $res = ['success' => false, 'message' => 'Duplicate Sub Account Name'];
            echo json_encode($res);
            exit;
        }
        ...
        try {
            ...
            if ($subAccountId == '') {
                unset($data['sub_acc_id']);
                $data['status']    = 'a';
                $data['AddBy']     = $this->session->userdata("userId");
                $data['AddTime']   = date('Y-m-d H:i:s');
                $data['branch_id'] = $this->brunch;

                $this->db->insert('tbl_account_sub', $data);
            } else {
                unset($data['sub_acc_id']);
                $data['UpdateBy']   = $this->session->userdata("userId");
                $data['updatetime'] = date('Y-m-d H:i:s');
                $this->db->where('sub_acc_id', $subAccountId)->update('tbl_account_sub', $data);
            }
    ...
    public function getSubAccounts()
    {
        ...
        if (isset($data->account_id) && $data->account_id != '') {
            $clause = " and sb.account_id = '$data->account_id' ";
        }
        // if (isset($data->unite_id) && $data->unite_id != '') {
        //     $clause = " and ua.unite_id = '$data->unite_id' ";
        // }

        $result = $this->db->query("SELECT sb.*,ac.Acc_Name
        FROM tbl_account_sub sb
        LEFT JOIN tbl_account ac on ac.Acc_SlNo = sb.account_id
        WHERE sb.status = 'a'
        $clause
        order by sb.sub_acc_id desc")->result();
    ...
    public function deleteSubAccount()
    {
        ...
        $updateData = [];
        $updateData['status'] = 'd';
        $updateData['UpdateBy']   = $this->session->userdata("userId");
        $updateData['updatetime'] = date('Y-m-d H:i:s');

        $this->db->where('sub_acc_id', $data->sub_acc_id)->update('tbl_account_sub', $updateData);

        echo json_encode(['success' => true, 'message' => 'Sub Account delete successfully']);
    }
}
