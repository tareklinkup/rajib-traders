					...
					<div class="col-md-5 col-md-offset-1">
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-xs-11">
								...
								<v-select v-bind:options="accounts" @input="getSubAccounts" v-model="selectedAccount" label="Acc_Name" v-if="accounts.length > 0"></v-select>
							</div>
						...
						<div class="form-group" style="display:none;" :style="{display:subAccounts.length > 0 ? '' : 'none'}">
							<label class="col-md-4 control-label">Sub-Account</label>
							<label class="col-md-1">:</label>
							<div class="col-md-6 col-xs-11">
								<v-select v-bind:options="subAccounts" v-model="selectedSubAccount" label="sub_account_name"></v-select>
							</div>
							<div class="col-xs-1" style="padding-left:0;margin-left: -3px;">
								<a href="/add_sub_account" target="_blank" class="add-button"><i class="fa fa-plus"></i></a>
							</div>
						</div>
					</div>
	...
	new Vue({
		...
		methods: {
			...
			getSubAccounts() {
				...
				axios.post('/get_sub_account', {
					account_id: this.selectedAccount.Acc_SlNo
				}).then(res => {
			...
			addTransaction() {
				...
				if (this.selectedSubAccount == null || this.selectedSubAccount.sub_acc_id == undefined) {
					this.transaction.sub_acc_id = '';
				} else {
					this.transaction.sub_acc_id = this.selectedSubAccount.sub_acc_id;
				}
			...
			editTransaction(transaction) {
				...
				if (this.transaction.sub_acc_id != '') {
					this.selectedSubAccount = {
						sub_acc_id: transaction.sub_acc_id,
