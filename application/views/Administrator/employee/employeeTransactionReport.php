...
<div id="salaryReport">
        ...
        <div class="col-md-12">
            <form class="form-inline" @submit.prevent="getEmployeeTransactions">
                ...
                <div class="form-group">
                    ...
                    <select v-model="filter.transaction_for" class="form-control" @input="onChangeTrnsactionFor"
                        style="padding: 0px 1px;width:150px">
    ...
    <div class="row" style="margin-top: 10px;display:none;"
        v-bind:style="{display: transactions.length > 0 ? '' : 'none'}">
        <div class="col-md-12">
        ...
        <div class="col-md-12">
            <div class="table-responsive" id="reportContent">
                ...
                <div style="display:none;" v-bind:style="{display: transactions.length > 0 ? '' : 'none'}">
                    <h3 v-if="filter.transaction_for == 'Salary'" style="text-align:center;">Employee Salary Records
                    </h3>
                    <h3 v-if="filter.transaction_for == 'Advance'" style="text-align:center;">Employee Advance Records
                    </h3>
                    ...
                    <table class="table table-bordered table-condensed">
                        ...
                        <tbody>
                            <tr v-for="(payment, sl) in transactions">
                                ...
                                <td v-if="filter.transaction_for == 'Salary'" style="text-align:right;">
                                    {{ payment.salary }}</td>
                                <td v-if="filter.transaction_for == 'Salary'" style="text-align:right;">
                                    {{ payment.Leave_deduction }}</td>
                                <td v-if="filter.transaction_for != 'Loan'" style="text-align:right;">
                                    {{ payment.advance_adjust }}</td>
                                <td style="text-align:right;">{{ payment.paid }}</td>
                                <td style="text-align:right;">{{ payment.received }}</td>
                                <td v-if="filter.transaction_for != 'Advance'" style="text-align:right;">
                                    {{ payment.balance }}</td>
                            </tr>
                        </tbody>
                        <tfoot v-if="filter.transaction_for == 'Advance'">
                            <tr style="font-weight:bold;">
                                ...
                                <td style="text-align:right;">
                                    {{ transactions.reduce((prev, curr) => { return prev + parseFloat(curr.advance_adjust)}, 0).toFixed(2) }}
                                </td>
                                <td style="text-align:right;">
                                    {{ transactions.reduce((prev, curr) => { return prev + parseFloat(curr.paid)}, 0).toFixed(2) }}
                                </td>
                                <td style="text-align:right;">
                                    {{ transactions.reduce((prev, curr) => { return prev + parseFloat(curr.received)}, 0).toFixed(2) }}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
