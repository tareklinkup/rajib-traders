				...
				<div class="col-xs-6">
					<table class="balancesheet-table">
						...
						<tbody>
							...
							<tr>
								<td>Employee Received</td>
								<td style="text-align:right;">{{ totalEmployeeReceiveds | decimal }}</td>
							</tr>
							<tr>
	...
	new Vue({
		...
		methods: {
			async getStatements() {
				...
				await this.getEmployeeReceiveds();
			...
			async getEmployeePayments() {
				await axios.post('/get_employee_transaction', this.filter)
					.then(res => {
						this.employeePayments = res.data.filter((obj) => {
							return obj.transaction_type == 'Payment'
						});
					})
			},
			async getEmployeeReceiveds() {
				await axios.post('/get_employee_transaction', this.filter)
					.then(res => {
						this.employeeReceiveds = res.data.filter((obj) => {
