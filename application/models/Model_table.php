...
class Model_Table extends CI_Model
{
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(et.received_amount), 0) from tbl_employee_transaction et
                where et.Branch_id = " . $this->session->userdata('BRANCHid') . "
                and et.status = 'a'
                and et.transaction_type = 'Receive'
                " . ($date == null ? "" : " and et.date < '$date'") . "
            ) as employee_received,
            ...
            (
                select ifnull(sum(et.payment_amount), 0) from tbl_employee_transaction et
                where et.Branch_id = " . $this->session->userdata('BRANCHid') . "
                and et.status = 'a'
            ...
            /* total */
            (
                select received_sales + received_customer + received_supplier + received_cash + bank_withdraw + loan_received + loan_initial_balance + invest_received + sale_asset + employee_received
            ) as total_in,
            (
    ...
    public function supplierDue($clauses = "", $date = null)
    {
        ...
        $supplierDues = $this->db->query("
            ...
            (select ifnull(sum(ct.In_Amount), 0.00) from tbl_cashtransaction ct
                where ct.status = 'a'
                and ct.Tr_Type = 'In Cash'
                and ct.Acc_SlID = 9
                and s.Supplier_SlNo = 1
                " . ($date == null ? "" : " and ct.Tr_date < '$date'") . "
            ) as ct_received,
            
            (select invoicePaid + cashPaid + ct_received) as paid,
