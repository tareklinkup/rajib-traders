					...
					<div style="width:50%;border:1px solid black;position:relative;">
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="employeeReceiveds.length > 0">
									<tr>
										<td class="sub-heading">Employee Received</td>
										<td class="sub-value">{{ totalEmployeeReceiveds }}</td>
									</tr>
									<tr v-for="payment in employeeReceiveds">
	...
	new Vue({
		...
		computed: {
			...
			totalEmployeeReceiveds() {
				return this.employeeReceiveds.reduce((prev, curr) => {
					return prev + parseFloat(curr.totalAmount)
				}, 0).toFixed(2);
			},
			totalInitialLoan() {
		...
		methods: {
			...
			getCashReceived() {
				...
				axios.post('/get_cash_transactions', filter)
					.then(res => {
						let transactions = res.data.filter((obj) => {
			...
			getEmployeePayments() {
				axios.post('/get_employee_transaction', this.filter)
					.then(res => {
						let payments = res.data.filter((obj) => {
							return obj.transaction_type == 'Payment'
						});
			...
			getEmployeeReceiveds() {
				axios.post('/get_employee_transaction', this.filter)
					.then(res => {
						let payments = res.data.filter((obj) => {
							return obj.transaction_type == 'Receive'
						});
						payments = _.groupBy(payments, 'Employee_SlNo');
						payments = _.toArray(payments);
						payments = payments.map(payment => {
							payment[0].totalAmount = payment.reduce((p, c) => {
								return p + parseFloat(c.received_amount)
							}, 0);
							return payment[0];
						})
						this.employeeReceiveds = payments;
